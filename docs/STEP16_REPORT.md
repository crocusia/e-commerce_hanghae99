...
## 현재 구조 개선 설계
### 1. 재고 예약 비동기 처리
초기에 설계 고민 시, 주문 생성과 재고 예약을 동기로 처리해야 한다고 생각함

- 원인 : 특정 인기 상품에 트래픽이 몰려 락 처리가 늦어져 재고 예약이 지연되는 상황에서,
재고 예약이 완료되지 않았음에도 **비동기**처리로 인해 결제가 생성되는 상황을 고려.
...
### 단점
- 높은 러닝커브
- DB 커밋 및 이벤트 발행의 원자성 부족 -> 아웃박스 패턴 필요
- 메시지 중복 처리
- 디버깅 복잡성 증가
- 운영 및 유지보수 비용 증가
- 처리 지연
